...
**Answer** dataframe with data on the precincts in prince george's county. some of the columns in this data are named precinct_id, precinct_name, congress, legis, school, polling_id, shape_length, shape_area and shape.
...
**Answer** dataframe with all the precincts in prince george's county. shows their ID, name, shape, etc.
...
**Answer** a map of prince george's county that shows all its precincts
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** it is a dataframe with every county in the United States. It contains their codes, names, shapes, areas, etc.
```{r}
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** it filtered out all the US counties that aren't in Maryland
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** map of maryland, but there is no chesapeake bay, instead the surrounding counties are expanded to fill it in.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** the precincts that were previously not colored gray in the first map are now colored gray.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** dataframe that shows voter turnout per precinct. also shows the number of early, absentee, provisional and eligible votes.
...
**Task** Run the following code. Describe the output in the space below.
**Answer** it created a dataframe that combined the pg_precincts dataframe and the pg_turnout dataframe. It joined them by precinct ID. Now it shows the voter turnout per precinct. In precincts that had no voter turnout, it returned an NA. 
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** it output a map that shows how many eligible voters are in each precinct of Prince George's County.
...
**Answer** it loaded all the librarys and the census api
...
library(janitor)
library(tidycensus)
census_api_key("560e1d5588de380d3db8f5d13e7248b252f68cd4")
```
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** it shows the eligible voters per political party in each county.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** it shows the population estimate per county in maryland. there's also a margin of error column, however each row has an NA in it. it also ordered alphabetically per county.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** it's the same dataframe as before however it added multipolygon shapes for each row of data.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** dataframe showing the voters per party in each county
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** it changed the name of the "NAME" column in the md_county_population to "County"
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** it made a dataframe with both the population data per county and the eligible voters per party per county. the multipolygon data is there as well
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** the dataframe shows the population and multipolygon data for each maryland county.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** it added a voters per 10k column that shows the total voters divided by the population times 10000. it shows this number for each county.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** map of maryland showing the voters per 10k in each county. the color gradient is different shades of blue. the lighter the shade the higher the voter count is.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** this changed the color gradient from different shades of blue to a gradient of numerous different colors. the colors stand out from each other making it easier to discern slight differences in voters. 
...
**Answer** the colors look more or less the same. they seem to pop a little bit more but i'm also running on little sleep so i'm not sure.
...
### Task: Run code
**Task** Run the following code. Describe the output in the space below.
**Answer** it added a column that shows the number of unaffiliated voters per 10k people in each county.
...
**Answer** it shows a map of maryland that shows the number of unaffiliated voters per 10k people in each county. it uses the same color gradient as the previous map that was made. 
