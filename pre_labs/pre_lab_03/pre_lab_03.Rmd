...
**Answer** 1,640 rows have problems. The problem that R is describing is that it is expecting to see TRUE/FALSE in the provisional and limited columns, respectively. However, the data table is showing something different like NA/NA or FALSE/FALSE.
...
**Answer** I don't see it reporting any parsing errors.
...
**Answer** I don't see an amount, name or id column. However I would assume an amount field would be <dbl>, a name column would be <chr> and an id column would be <chr> despite an id typically being a number. This is because id numbers are not used for data analysis... for example, nobody wants to find the average id number in a dataset.
```{r}
...
**Answer**: There are 402345 results where mail is NA. The implication is that it was not specified whether or not the votes were via mail or not.
...
**Answer** There are 12 registrations for January 1900. The registrations from 1900 have to be mistakes, especially since you have to generally be 18 to vote. The one from 1933 may be the same way as well. The rest of the registration dates are more likely to be legitimate given they are from 1949 onward. My guess as to why there are so many registrations in 1966 is that there may have been a significant election that year.
```{r}
yadkin_voters %>% 
...
**Answer** The temporary registrant category is interesting to me. Does this mean that the voters that submitted those ballots will no longer be registered to vote after the election? If so, does that make their vote legitimate? I have never heard of people un-registering to vote.
...
**Answer** the amount field is listed as a character not a number, a lot of the city data is misspelled and not all lowercase/uppercase, some of the zipcodes are 5 digits while some are 9 digits.
...
**Answer** It added an x in front of the 1 in the name, renaming the field "x1_linenumber". This makes the name of the headers more consistent since it was the only header that started with a numeral.
...
It changed the x1_linenumber header to simply linenumber.
```{r}
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
...
**Answer** It says the there is an invalid 'type' of argument when computing the line "total_amount = sum(amount)." This means the the amount field is being read as a character not a number.
...
**Answer** there is one row of data that was duplicated, making two duplicates... i guess? The donor name is Derrick Hamilton
...
**Answer** all the zip codes are now five digits instead of nine.
...
**Task** Run the following codeblock to use str_tot_title() to standarize capitalization in the "city" field. How many misspellings of Arnold remain after running this code? Answer below. Add a description of what this code does to your reference notebook.
It made all of the city names have their first letter be capitalized.
```{r}
# cleaning function
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
...
**Answer** It fixed all of the misspellings "Conowing" to "Conowingo" However there is still a "Conowingoo"
...
**Answer** It fixed the misspelling "Conowingoo" so it is now spelled as "Conowingo." All 13 are now spelled properly.
```{r}
# cleaning function
cleaned_conowingo <- conowingo %>%
  clean_names() %>% 
