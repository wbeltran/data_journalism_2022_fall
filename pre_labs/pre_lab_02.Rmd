...
```{r}
primary_18_w_election_day <- primary_18 %>%
...
**Answer** It is selecting a handful of specific columns of data from primary_18. The mutate function is creating a new column of data that is showing how many votes for a candidate were made on election day. This was done by dividing the election_day column by the votes column.
...
**Answer** the new percent_election_day column was multiplied by 100 to represent a percentage instead of a decimal. The column was then arranged so that the highest percent was shown first, descending in order to the lowest.
...
**Answer** This arranged the percent_election_day column from lowest percent descending to the highest percent.
...
**Answer**
str_to_lower changes all letters in a string value to lower case.
str_to_title capitalizes the first letter of each word in a string.
...
**Answer** The mutate function is checking the value of the amount column for each row. The value of the contribution determines what amount category it is sorted into. For example, it the amount is greater than $2900, then the amount category it's put into is "More than 2900."
...
**Answer** if a new variable is not made, then the result will only be temporary. Creating a new variable stores it in the environment so it can be viewed later.
...
**Answer** The month column is showing which month of the year the total amount of contributions in the next column occurred in. It also has the specific date as the first of the month, even though this should be the aggregate contributions for the month, not just the first of the month. I am guessing it is doing that because it is programmed to show a yyyy-mm-dd style date... however the day does not actually matter.
