...
**Answer** it's a blank canvas, box shaped.
...
**Task** Run the following code. Explain what you see after running the code in the space below.
**Answer** it is a bar chart, showing the number of difference of registered voters for the top 10 counties. the counties are sorted alphabetically along the x-axis. the y-axis is the count.
...
**Answer** the x-axis has been sorted to have the lowest difference on the left, and the counties ascend to the highest difference on the right. more visually appealing, gets to the point better.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** this flipped the data between the two axises, making a horizontal bar chart. this also made the county names easier to read, as they are no longer overlapping.
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** the black tick marks along the axises were removed. also the background color changed from gray to white.
...
**Answer** the chart now looks like it published in the economist. it made the background blue and made the font size larger.
```{r}
maryland_top_counties %>%
  ggplot() +
  geom_bar(aes(x=reorder(County,TOTAL_DIFF), weight=TOTAL_DIFF)) +
...
**Task** Run the following code. Explain what you see after running the code in the space below. What has changed from the last codeblock?
**Answer** it added a chart title and changed the names of the axises. it also added a caption to attrbute the data source.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** it is showing the number of winred contributions per day. the data is arranged by date, descending from April 1, 2022 onward.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** it shows a line chart that depicts the number of winred contributions per day from april through june.
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** it added more tick marks along the x-axis. instead of the tick marks showing the beginning of each month, it is now showing a tick mark every week. makes it easier to see when a y-axis datapoint occurred. 
...
**Task** Run the following code. Explain what you see after running the code in the space below. 
**Answer** it tilted the individual date labels along the x-axis by 45 degrees so that they don't overlap, making them easier to read.
```{r}
md_winred_by_date %>%
  ggplot() + 
  geom_line(aes(x=date, y=total_contributions)) + 
